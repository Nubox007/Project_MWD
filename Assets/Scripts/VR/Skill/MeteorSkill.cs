...
public class MeteorSkill : MonoBehaviour
{
    ...
    [SerializeField] private ParticleSystem[] meteortornadoParticleSystem; // 토네이도 파티클 시스템
    ...
    private void Update()
    {
        for (int i = 0; i < meteortornadoParticleSystem.Length; i++)
        {
            if (!isMoving && meteortornadoParticleSystem[i].isPlaying) // 파티클 시스템이 작동 중일 때만 움직임
            {
                MoveTornado();
            }
        }
    ...
    private IEnumerator MoveBetweenPoints()
    {
        ...
        foreach (var particleSystem in meteortornadoParticleSystem)
        {
            particleSystem.Stop();
